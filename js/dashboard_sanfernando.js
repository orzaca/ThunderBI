/* dashboard_SanFernando.js - VERSIÓN FINAL: INTERACTIVIDAD Y SEPARACIÓN COMPLETA DE CACHÉS (FIXED FINAL TIMEZONE) */
...
function ayerYYYYMMDD() { return sumarDias(hoyYYYYMMDD(), -1); }
...
const btnRefrescar = document.querySelectorAll('.btn-fecha-primario i.fa-rotate-right').length > 0 ? document.querySelector('.btn-fecha-primario i.fa-rotate-right').closest('button') : null;
...
async function mapWithConcurrency(items, workerFn, concurrency = 6) {
  ...
  const runners = new Array(concurrency).fill(0).map(async () => {
    while (idx < items.length) {
      const i = idx++;
      try { results[i] = await workerFn(items[i], i); } catch (e) { results[i] = null; }
    }
  });
...
/* --- 5. API CALLS --- */
function toTracktikDesde(fechaYYYYMMDD){ return `${fechaYYYYMMDD}T00:00:00-05:00`; }
function toTracktikHasta(fechaYYYYMMDD){ return `${fechaYYYYMMDD}T23:59:59-05:00`; }

async function fetchReportsMetaByClient(token, clientId, desdeYYYY, hastaYYYY) {
  const limit = 100; let offset = 0; const all = [];
  const desde = toTracktikDesde(desdeYYYY); const hasta = toTracktikHasta(hastaYYYY);
  while (true) {
    const filtros = { limit, offset, "account.id:in": clientId, "reportDateTime:after": desde, "reportDateTime:before": hasta, include: "account,reportTemplate,createdBy" };
    const url = `${CONFIG_CLIENTE.apiUrl}reports?${new URLSearchParams(filtros).toString()}`;
    const r = await fetch(url, { headers: token ? { "Authorization": `Bearer ${token}` } : {} });
    if (!r.ok) break;
    const json = await r.json();
    const data = json?.data ?? []; all.push(...data);
    if (data.length < limit) break; offset += limit;
  }
  return all;
}
...
async function fetchReportDetailsForList(metaList, token, concurrency = 6) {
  const worker = async (meta) => {
    const reportId = meta.id || meta.uuid; if (!reportId) return null;
    const url = `${CONFIG_CLIENTE.apiUrl}reports/${reportId}?include=reportFields`;
    const r = await fetch(url, { headers: token ? { "Authorization": `Bearer ${token}` } : {} });
    if (!r.ok) return null;
    const json = await r.json(); const rep = json.data || json;
...
function findPictureRefsInFields(fields) {
  ...
  fields.forEach(f => {
    ...
    if (valStr.startsWith(CONFIG_CLIENTE.apiUrl.replace('/rest/v1', '').replace(/\/$/, '') + "/rest/v1/files") || valStr.startsWith(CONFIG_CLIENTE.apiUrl.replace('/rest/v1', '').replace(/\/$/, '') + "/rest/v1/media") || f.type === "picture" || f.type === "camera") {
...
async function pintarTablaReportes(reportes) {
  ...
  for (const r of listaOrdenada) {
    ...
    fields.forEach(f => {
      ...
      if (valStr.startsWith(CONFIG_CLIENTE.apiUrl.replace('/rest/v1', '').replace(/\/$/, '') + "/rest/v1/files") || valStr.startsWith(CONFIG_CLIENTE.apiUrl.replace('/rest/v1', '').replace(/\/$/, '') + "/rest/v1/media") || type === "picture" || type === "camera" || label.includes("GPS Location") || label.includes("Ubicación GPS")) {
           return; 
      }
...
function pintarTablaEmpleados(empleados) {
  ...
  if (!empleados.length) {
    ...
    const td = document.createElement("td"); td.colSpan = 4; td.textContent = "No hay empleados."; td.style.textAlign="center"; td.style.padding="16px";
    tr.appendChild(td); tbodyEmpleados.appendChild(tr); return;
  }
  empleados.forEach(e => {
    const tr = document.createElement("tr");
    tr.innerHTML = `
...
async function cargarReportesYEmpleadosPorRango(desdeYYYY, hastaYYYY) {
  ...
  } catch (e) { console.error("Error en la carga principal:", e); } finally { document.getElementById("loader-global-cliente").classList.add("oculto"); }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnRapidos.forEach(b => b.addEventListener("click", () => {
    const tipo = b.getAttribute("data-rango"); let desde = hoy, hasta = hoy;
    ...
    inputDesde.value = desde; inputHasta.value = hasta; cargarReportesYEmpleadosPorRango(desde, hasta);
  }));
